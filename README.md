...
- **AI 任务拆解**：(New!) 输入模糊的任务想法，AI 能够自动将其转化为规范的任务标题，并生成具体的执行步骤计划。
- **任务状态**：标记任务完成状态，记录每个任务的专注时长。
- **任务历史**：查看已完成或正在进行的任务列表。
...
### 缺点与不足 (Cons)
1. **代码结构**：
   - `Dashboard.tsx` 文件承担了过多的职责（UI、状态管理、计时器逻辑），成为了一个 "God Component"，难以维护。
   - 样式大量使用行内样式 (`style={{...}}`)，导致 JSX 代码臃肿，复用性差。
2. **数据持久化**：
   - 目前完全依赖 `localStorage`。虽然简单，但容易因清理缓存而丢失数据，且不适合存储大量历史数据。
   - `BreakView` 和 `Dashboard` 之间通过 `localStorage` 同步状态，这种方式不够健壮。
3. **功能体验**：
   - 缺乏声音提醒（结束提示音）。
   - 任务管理功能较弱（无法编辑、无优先级、无分类）。

### 改进建议 (Roadmap)

#### 1. 代码重构
- **样式抽离**：引入 CSS Modules 或 Tailwind CSS，移除行内样式。
- **逻辑抽离**：创建自定义 Hooks（如 `useTimer`, `useTask`, `useConfig`）来管理状态，简化组件代码。
- **状态管理**：引入 Zustand 或 Redux Toolkit 管理全局状态，优化组件通信。

#### 2. 数据存储升级
- 引入 **SQLite** (通过 `tauri-plugin-sql`) 或使用文件系统存储 (JSON/CSV)，确保数据安全和迁移能力。
- 实现更完善的数据模型，记录每次专注的详细时间戳，以便生成精细的报表。

#### 3. 功能增强
- **声音提醒**：添加白噪音背景音和计时结束提示音。
- **本地 AI 支持**：支持 Ollama 等本地大模型，增强隐私保护并降低成本。
- **任务增强**：支持任务拖拽排序、添加标签/项目分类。
- **系统托盘**：添加系统托盘图标，支持在不打开主窗口的情况下查看倒计时或控制计时。
